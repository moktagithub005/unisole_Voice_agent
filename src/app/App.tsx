...
import {
  indianStockAdvisorScenario,
  advisorBrandName,
} from "@/app/agentConfigs/indianStockAdvisor";
...
// Versioning for workspace localStorage.
const WORKSPACE_VERSION_KEY = 'workspace_version';
const INDIAN_STOCK_VERSION = 'indian_stock_v1';

function App() {
  const searchParams = useSearchParams()!;

  // One-time migration for this demo scenario.
  React.useEffect(() => {
    if (typeof window === 'undefined') return;
    const scenario = (new URL(window.location.href).searchParams.get('agentConfig')) || '';
    if (scenario !== 'indianStockAdvisor') return;
    const currentVersion = window.localStorage.getItem(WORKSPACE_VERSION_KEY);
    if (currentVersion !== INDIAN_STOCK_VERSION) {
      window.localStorage.removeItem('workspaceState');
      window.localStorage.setItem(WORKSPACE_VERSION_KEY, INDIAN_STOCK_VERSION);
  ...
  useEffect(() => {
    ...
    if (finalAgentConfig !== 'indianStockAdvisor') {
  ...
  const connectToRealtime = async () => {
    ...
    if (sdkScenarioMap[agentSetKey]) {
      ...
      try {
        ...
        const guardrails = [createModerationGuardrail(advisorBrandName)];
  ...
  const updateSession = (shouldTriggerResponse: boolean = false) => {
    ...
    // Send an initial greeting trigger so the agent starts in Hindi-first Hinglish.
    if (shouldTriggerResponse) {
      sendSimulatedUserMessage('namaste');
    }
  }
    ...
    <div className="text-base flex flex-col h-screen bg-gray-100 text-gray-800 relative">
      <div className="p-5 text-lg font-semibold flex justify-between items-center">
        ...
        >
          ...
          <div>
            {advisorBrandName} <span className="text-gray-500">Agent</span>
          </div>
        </div>
